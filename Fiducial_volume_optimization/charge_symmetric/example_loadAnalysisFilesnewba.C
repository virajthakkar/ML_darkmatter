...
*/
   eventChain->Add( Form("%s/merge_Prodv5-3-5_z%dlite_*_%s*.root/rqDir/eventTree",base.c_str(),z2,type.c_str()) );
   mergeChain->Add( Form("%s/merge_Prodv5-3-5_z%dlite*.root/rqDir/zip%d",base.c_str(), z2, z) );
   calibChain->Add( Form("%s/calib_Prodv5-3-5_z%dlite*.root/rrqDir/calibzip%d",base.c_str(), z2,z) );
   calibEvent->Add( Form("%s/calib_Prodv5-3-5_z%dlite*.root/rrqDir/calibevent",base.c_str(), z2) );

   cout<<"eventchain"<<endl;
cout<<Form("%s/merge_Prodv5-3-5_z%dlite_*_%s*.root/rqDir/eventTree",base.c_str(),z2,type.c_str());

cout<<"mergeChain"<<endl;
cout<<Form("%s/merge_Prodv5-3-5_%s*_%s_z%dlite*.root/rqDir/zip%d",base.c_str(), week.c_str(), type.c_str(), z2, z);
